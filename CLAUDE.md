...
│       ├── nhap-lieu/+page.svelte       # Nhập tay (auto-focus) + Scrape hàng loạt theo dải ngày
...
Schema thực thi inline trong `database.js` (`getDb()` chạy migrate lần đầu, bật WAL + `foreign_keys = ON`) — đây là nguồn schema duy nhất.

**App chỉ dùng dữ liệu Miền Bắc.** Cột `province` giữ lại trong bảng nhưng **luôn = `mien-bac`** (gán cứng trong `saveDraw`); không có UI/logic đa miền. `saveDraw(draw_date, prizes)` / `updateDraw(id, draw_date, prizes)` / `findDraw(draw_date)` không nhận tham số miền.
...
2. Tới giờ: `scrapeResult(today)` → `scraper.js` **race song song 3 nguồn** (minhchinh → xosothantai → ketquaxoso) bằng `Promise.any`, lấy nguồn trả về trước. Thất bại thì retry tối đa 3 lần (5, 10 phút).
...
Endpoint thủ công để cào lại: `POST /api/daily-scrape` (body `{ dates? }`, mặc định hôm nay + hôm qua), hoặc `GET /api/scrape?date=` (chỉ thử, không lưu).
...
- Trang **`/trang-thai`** liệt kê ngày hụt dữ liệu (30 ngày) + nhật ký 60 lần gần nhất, có nút **"cào bù"** gọi `POST /api/daily-scrape` với `{ dates }` cụ thể rồi `invalidateAll()`.
- `getMissingDates(days)` (`scrape-log.js`) so ngày kỳ vọng với `draws` để tìm ngày thiếu.
...
`POST /api/daily-scrape` nhận `{ dates? }` — mặc định [hôm nay, hôm qua] (Miền Bắc). Mẫu cron ngoài: `.github/workflows/daily-scrape.yml` (11:55 UTC = 18:55 VN, cần secret `APP_URL` + `CRON_SECRET`).
...
## Cấu trúc menu (2 cấp, trong `+layout.svelte`)

- **Dashboard** (`/`)
- **Thống kê** (dropdown): Bảng thống kê · Khuyến nghị · Lô Gan · Cầu Lô · Dàn Đề · Dự Đoán · Nghiên cứu
- **Dữ liệu** (dropdown): Nhập liệu · Lịch · Lịch sử · Trạng thái

Desktop: dropdown bấm mở/đóng (có backdrop click-outside). Mobile: hamburger, nhóm cha hiển thị dạng accordion.
...
| `/nhap-lieu` | Nhập tay (auto-focus) + Scrape hàng loạt nhiều ngày (gọi `/api/daily-scrape`) |
